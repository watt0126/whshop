...
use app\store\model\GoodsExpert as GoodsExpertModel;
...
class Goods extends GoodsModel
{
    ...
    public function add(array $data)
    {
		...
		if (!isset($data['experts']) || empty($data['experts'])) {
		    $this->error = '请选择关联专家';
		    return false;
		}
        ...
        try {
			...
			// 关联专家
			$this->addGoodsExperts($data['experts']);
            $this->commit();
            return true;
        } catch (\Exception $e) {
    ...
    public function edit($data)
    {
        ...
        return $this->transaction(function () use ($data) {
            ...
            $this->addGoodsImages($data['images']);
			// 关联专家
			$this->addGoodsExperts($data['experts'],true);
            return true;
        });
    }
	/**
	 ...
	 * @throws \think\exception\PDOException
	 */
	private function addGoodsExperts($experts,$isUpdate = false)
	{
	    $model = new GoodsExpertModel;
		$isUpdate && $model->removeAll($this['goods_id']);
		
	    $data = array_map(function ($expert_id) {
	        return [

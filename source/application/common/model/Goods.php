...
class Goods extends BaseModel
{
    ...
    protected $append = ['goods_sales','goods_views'];
	 ...
	 * @return mixed
	 */
	public function getGoodsViewsAttr($value, $data)
	{
	    return $data['views_initial'] + $data['views_actual'];
	}
    /**
     * 获取器：单独设置折扣的配置
     * @param $json
	...
	{
	    return $this->hasMany('GoodsExpert')->order(['id' => 'asc']);
	}
    /**
     * 关联运费模板表
     * @return \think\model\relation\BelongsTo
    ...
    public function getList($param)
    {
        ...
        isset($params['is_choice']) && $params['is_choice'] > -1 && $filter['is_choice'] = $params['is_choice'];
        ...
        } elseif ($params['sortType'] === 'views') {
            $sort = ['goods_views' => 'desc'];
    ...
    protected function setGoodsListData($data, $isMultiple = true, callable $callback = null)
    {
        ...
        foreach ($dataSource as &$goods) {
			...
			// 专家头像、缩略图
			foreach($goods['expert'] as $k=>$e){
				$goods['expert'][$k]['avatar_image'] = $e['expert']['avatar']['file_path'];
				$goods['expert'][$k]['expert_image'] = $e['expert']['image']['file_path'];				
			}
            // 商品默认规格
